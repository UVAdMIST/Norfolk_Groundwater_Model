...
train_X = np.concatenate((gwl_input[:n_train, :], rain_super_values[:n_train, :]),
                         axis=1)
test_X = np.concatenate((gwl_input[n_train:, :], rain_super_values[n_train:, :]),
                        axis=1)
...
test_dates_t1 = fcst_data[['Datetime', 'rain(t+1)']]
...
test_dates_t9 = fcst_data[['Datetime', 'rain(t+9)']]
...
test_dates_t18 = fcst_data[['Datetime', 'rain(t+18)']]
...
df_t1 = df_t1.set_index("Datetime")
df_t1 = df_t1.rename(columns={'rain(t+1)': 'fcst rain(t+1)'})
...
df_t9 = df_t9.set_index("Datetime")
df_t9 = df_t9.rename(columns={'rain(t+9)': 'fcst rain(t+9)'})
...
df_t18 = df_t18.set_index("Datetime")
df_t18 = df_t18.rename(columns={'rain(t+18)': 'fcst rain(t+18)'})
...
all_data_df_clean = all_data_df[['GWL', 'Precip.', 'Pred. GWL t+1', 'Pred. GWL t+9', 'Pred. GWL t+18']]
...
for start, stop in zip(starts, stops):
    ...
    for plot_num, ax in enumerate(fig.axes):
        ...
        period_data[[observed, storm_pred, fcst_pred]] \
            .plot(ax=ax, color=['k', 'b', 'r'], style=["-", '-.', '-.'],
                  label=["Obs. GWL", "Pred. GWL Storm", "Pred. GWL Fcst."], legend=None)
    ...
    ax.legend(lines + lines2,
              ("Obs. GWL", "Pred. GWL", "Pred. GWL Fcst.", "Fcst. Precip.",
